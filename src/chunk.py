...
class Chunk:
    ...
    def _collapse_water(self, neighbor_chunks: dict[tuple[int,int], "Chunk"], is_recollapse: bool = False) -> None:
        '''Classify raw WATER tiles into edge variants based on neighbors.'''
        WATER_VAL = Tile.WATER.value
        y0, x0 = self.index

        def get_tile(ly, lx) -> int:
            # Compute world tile coords
            wy = y0 + ly
            wx = x0 + lx

            # Compute which chunk that world tile belongs to
            
            cy = math.floor(wy / self.chunk_size) * self.chunk_size
            cx = math.floor(wx / self.chunk_size) * self.chunk_size

            # Local coords within that chunk
            local_y = wy - cy
            local_x = wx - cx

            nidx = (cy, cx)
            if nidx == (y0, x0):
                t = int(self.chunk[local_y, local_x])
            elif nidx in neighbor_chunks:
                t = int(neighbor_chunks[nidx].chunk[local_y, local_x])
            else:
        ...
        for y in range(self.chunk_size):
            for x in range(self.chunk_size):
                # on recollapse only iterate on edges
                if is_recollapse and not (y == 0 or y == self.chunk_size - 1 or x == 0 or x == self.chunk_size - 1):
                    continue
    ...
    def _generate_chunk_water_image(self, is_recollapse: bool = False) -> None:
        TREE_OVERHEAD = 96
        ...
        if is_recollapse:
            #on recollapse just blit the new borders on the old water images 
            for y in range(self.chunk_size):
                for x in range(self.chunk_size):
                    tile = Tile(chunk[y, x])
                    img = None
                    
                    if not (y == 0 or y == self.chunk_size - 1 or x == 0 or x == self.chunk_size - 1):
                        continue
                    ...
                    match tile:
                        case Tile.WATER_EXT:
                            for i in range(8):
                                ...
                                self.water[i].blit(img, (blit_x, blit_y))
    ...
    def _generate_chunk_image(self, is_recollapse: bool = False) -> None:
        if is_recollapse:
            self._generate_chunk_water_image(is_recollapse=is_recollapse) #On recollapse just generate the water image again, since the land doesnt change
        else:
            chunk = self.chunk
            TILE_W = 32 * SCALE
            TILE_H = 32 * SCALE  
            ...
            self._generate_chunk_water_image(is_recollapse=is_recollapse)
